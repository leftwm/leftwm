...
`leftwm-theme` and `leftwm-config` are not yet ported to OpenBSD, as they require a nightly Rust compiler to build.
The default config is generated by LeftWM when it is first started. You can install themes manually as described [here](#without-leftwm-theme).
