...
pub fn update(workspace: &Workspace, windows: &mut Vec<&mut Window>, tags: &mut Vec<Tag>) {
    ...
    let mut iter = windows.iter_mut();
    {
        if let Some(first) = iter.next() {
            first.set_width(workspace.width(window_count));
            first.set_height(height);
            first.set_x(workspace.x(window_count));
            first.set_y(main_y);
        }
    }

    //stack all the others
    let width_f = workspace.width(window_count) as f32 / (window_count - 1) as f32;
    let width = width_f.floor() as i32;
    let mut x = 0;
    for w in iter {
        w.set_height(workspace.height() - height);
        w.set_width(width);
        w.set_x(workspace.x(window_count) + x);
        w.set_y(stack_y);
        x += width;
    }
}
