...
fn tile_to_floating<C: Config, SERVER: DisplayServer>(
    manager: &mut Manager<C, SERVER>,
) -> Option<bool> {
    let width = manager.state.config.default_width();
    let height = manager.state.config.default_height();
    let window = manager.focused_window_mut()?;
    if window.must_float() {
        return None;
    }
...
fn toggle_floating<C: Config, SERVER: DisplayServer>(
    manager: &mut Manager<C, SERVER>,
) -> Option<bool> {
    let window = manager.focused_window_mut()?;
