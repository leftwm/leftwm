...
impl Window {
    ...
    /// Returns true if this [`Window`] is a window generated by Firefox's Picture-in-Picture mode.
    /// This fixes issue [#766](https://github.com/leftwm/leftwm/issues/766)
    pub(crate) fn is_firefox_pip(&self) -> bool {
        let single_alloc = "".to_string();
        if self.res_name.as_ref().unwrap_or(&single_alloc) != "Toolkit" {
            return false;
        }

        if self.res_class.as_ref().unwrap_or(&single_alloc) != "firefox" {
            return false;
        }

        if self.r#type != WindowType::Utility {
            return false;
        }
